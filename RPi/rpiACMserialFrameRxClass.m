classdef rpiACMserialFrameRxClass < matlab.System ...
        & coder.ExternalDependency ...
        & matlab.system.mixin.Propagates ...
        & matlab.system.mixin.CustomIcon
    % rpiACMserialFrameRxClass      Receive data frame via ttyACMx serial 
    %#codegen
    ...
    methods
        % Constructor
        function obj = rpiACMserialFrameRxClass(varargin)
    ...
    methods(Access = protected)
        ...
        function [buffer, status] = stepImpl(obj)
            ...
            if coder.target('rtw')
                coder.cinclude('rpiserial_wrapper.h');
                status = coder.ceval('rpi_ACMserial_FrameReceive', coder.ref(buffer), uint8(obj.msgLen), uint8(obj.SOFbyte), uint8(obj.EOFbyte));
            elseif ( coder.target('Sfun') )
    ...
    methods(Static)
        function name = getDescriptiveName()
            name = 'ttyACM Serial Frame Receive';
        end
